...
import { regis_f } from '../logic/function'
...
export default function Register() {
  ...
  const handlesubmit = () => {
    regis_f(Regis)
      .then(() => { alert('Login เถอะ สมัครเสร็จแล้ว');window.location='/' })
      .catch((err) => { alert(err.response.data) })
  }

  return (
    <section className='h-screen bg-zinc-700 flex justify-center items-center'>
      <form onSubmit={handlesubmit} className='max-w-sm p-10 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700'>
